...
import { NgPlural } from '@angular/common/src/directives/ng_plural';
...
@Component({
  ...
  styleUrls: ['./corporation.scss'],
  templateUrl: './corporation.html',
})
...
constructor(private route: ActivatedRoute, private modalService: NgbModal, private _corporationService: CorporationService, public _toasterService: ToasterService, vcr: ViewContainerRef, private router: Router) { //, private _menuService: BaMenuService
 ...
 ngOnInit() {
    ...
    this._corporationService.getVenture(this.id).subscribe(data => this.corporation = data,
    error => console.error('Error: ' + error),
    () => {}//console.log(JSON.stringify(this.corporation))
    // if  (this.corporation.primaryAccountManager != null || this.corporation.primaryAccountManager != 0) {
    //   this._corporationService.getUserById(this.corporation.primaryAccountManager) .subscribe(data => this.primaryAccountManager=data),
    //   error => console.error('Error: ' + error),
    //   () => {console.log(JSON.stringify(this.primaryAccountManager))}
    // }}
    );
    this._corporationService.getTop100Lists().subscribe(data => this.lists = data,
        error => console.error('Error: ' + error)
    );
    this._corporationService.getTop20Lists().subscribe(data => this.top20list = data,
        error => console.error('Error: ' + error)
    );    
    this._corporationService.getDealflowLists().subscribe(data => this.dealflowlist = data,
      error => console.error('Error: ' + error)
    );
    this.pageload = true;
  ...
  managerModalShow() {
    ...
    const contentComponentInstance = this.modalService.open(ManagerModal, {size: 'lg'}).componentInstance;
    contentComponentInstance.modalHeader = 'Add Account Manager';
    ...
    if (data.email != null) {
      this.addAccountManager(data.email,data.isPrimary);
    }
    });
  }
  ...
  championModalShow() {
    ...
    const contentComponentInstance = this.modalService.open(ChampionModal, {size: 'lg'}).componentInstance;
    contentComponentInstance.modalHeader = 'Add Champion';
    //I can even subscribe to outputs!
    contentComponentInstance.out.subscribe((data) => {
      // do sth with output
      console.log("Output from Modal: "+JSON.stringify(data));
  ...
  addTop100(id:Number,listName:String) {
    ...
    this.loading = true;
    this.error = false;
    this._corporationService.addToTop100(this.id,listName).map(res => {
      // If request fails, throw an Error that will be caught
      if(res.status == 204) {
        this.loading = false;
        this.error = true;
        this._toasterService.showWarning("corporation already exists in '"+listName+"'", "", 5000);
    ...
    addAccountManager(email:String, isPrimary: boolean) {
      this.loading = true;
      this.error = false;
      this._corporationService.addAccountManager(this.id,email,isPrimary).map(res => {
        if(res.status == 205) {
          this.loading = false;
          this.error = true;
          this._toasterService.showWarning("Account Manager '"+email+"' already exists in corporation.", "", 5000);
        } else if (res.status == 204) {
          this.loading = false;
          this.error = true;
          this._toasterService.showWarning("User with '"+email+"' does not exist.", "", 5000);
        }  else if (res.status == 206) {
          this.loading = false;
          this.error = true;
          this._toasterService.showWarning("User with '"+email+"' is already a champion.", "", 5000);
        } else if (res.status < 200 || res.status >= 300){
          this.loading = false;
          this.error = true;
          this._toasterService.showError("Could not add Account Manager, please try again.", "", 4000);
        } 
        else {
          this.loading = false;
          this._toasterService.showSuccess("Successfully added Account Manager '" +email+"'", "Success!", 2000);
          return res.json();
          
        }
      }).subscribe(data => {if (!this.error){this.corporation = data}},
        err => console.error('Error: ' + err),
            () => {}
        );
      }

      addChampion(email:String) {
        console.log("Add " +email+  " as champion to corporation ");
        this.loading = true;
        this.error = false;
        this._corporationService.addChampion(this.id,email).map(res => {
          if(res.status == 206) {
            this.loading = false;
            this.error = true;
            this._toasterService.showWarning("champion already exists '"+email+"'", "", 5000);
          } else if (res.status == 204) {
            this.loading = false;
            this.error = true;
            this._toasterService.showWarning("User with '"+email+"' does not exist.", "", 5000);
            ...
            this.loading = false;
            this.error = true;
            this._toasterService.showWarning("User with '"+email+"' is already an account manager for this corporation", "", 5000);
          } else if (res.status < 200 || res.status >= 300){
            this.loading = false;
            this._toasterService.showError("Could not add champion, please try again.", "", 4000);
            throw new Error('This request has failed ' + res.status);
          }
          else {
            this.loading = false;
            this._toasterService.showSuccess("Successfully added champion to corporation '" +email+"'", "Success!", 2000);
            return res.json();
  ...
  addTop20(id:Number,listName:String) {
    ...
    this.loading20 = true;
    this.error = false;
    this._corporationService.addToTop20(this.id,listName).map(res => {
      // If request fails, throw an Error that will be caught
      if(res.status == 204) {
  ...
  // setPrimaryAccountManager(id:Number,email:String,isPrimary:boolean) {
  //   console.log("Add "+id+ email+  " as parimary account manager to corporation ");
  //   this.loadingprimary = true;
  //   this.error = false;
  //   this._corporationService.setPrimaryAccountManager(this.id,email).map(res => {
  //     // If request fails, throw an Error that will be caught
  //     if(res.status == 204) {
  //       this.loadingprimary = false;
  //       this.error = true;
  //       this.showWarning("primary account manager already exists '"+email+"'", "", 5000);
  //     } else if (res.status < 200 || res.status >= 300){
  //       this.loadingprimary = false;
  //       this.showError("Could not add primary account manager, please try again.", "", 4000);
  //       throw new Error('This request has failed ' + res.status);
  //     }
  //     // If everything went fine, return the response
  //     else {
  //       this.loadingprimary = false;
  //       this.showSuccess("Successfully added account manager to corporation '" +email+"'", "Success!", 2000);
  //       return res.json();
        
  //     }
  //   }).subscribe(data => this.primaryAccountManager = data,
  //     err => console.error('Error: ' + err),
  //         () => console.log("Completed!")
  //     );
  //   }
  ...
  removeTop100(id:Number) {
    ...
    this._corporationService.removeFromTop100(this.id).subscribe(data => this.top100 = data,
    error => console.error('Error: ' + error),
      () => location.reload()
    );
...
//   accountManagersModal(company: any) {
//     var tmplist : any[] = [];
...
exportToPDF() {
  ...
  var teamSize = 10;
  if(this.corporation.background != null){
  if(this.corporation.background.length > 500){
    teamSize = 9;

  }else if(this.corporation.background.length > 1000){
    teamSize = 8;
  }
  }  
  var advSize = 10;
  if(this.corporation.advantage != null){
  if(this.corporation.advantage.length > 500){
    advSize = 9;

  }else if(this.corporation.advantage.length > 1000){
    advSize = 8;
  }
  }
  var caseSize = 10;
  if(this.corporation.caseStudy != null){
  if(this.corporation.caseStudy.length > 500){
    caseSize = 9;

  }else if(this.corporation.caseStudy.length > 1000){
    caseSize = 8;
  }
  }
  var summarySize = 12;
  if(this.corporation.blurb != null){
  if(this.corporation.blurb.length > 500){
    summarySize = 11;

  }else if(this.corporation.blurb.length > 1000){
    summarySize = 10;
  }  
  }
  var blurbFinal: any;
  if(this.corporation.blurb == null || this.corporation.blurb == ""){
    blurbFinal = "Information not provided by the startup"
  }else{
    blurbFinal = this.corporation.blurb;
  }
  var verticalsFinal: any;
  if(this.corporation.verticals == null || this.corporation.verticals == ""){
    verticalsFinal = "Information not provided by the startup"
  }else{
    verticalsFinal = this.corporation.verticals;
  }
  var headquartesFinal: any;
  if(this.corporation.city == null || this.corporation.city == ""){
    headquartesFinal = "Information not provided by the startup"
  }else{
    headquartesFinal = this.corporation.city;
  }
  var stageFinal: any;
  if(this.corporation.stage == null || this.corporation.stage == ""){
    stageFinal = "Information not provided by the startup"
  }else{
    stageFinal = this.corporation.stage;
  }
  var employeesFinal: any;
  if(this.corporation.employees == null || this.corporation.employees == ""){
    employeesFinal = "Information not provided by the startup"
  }else{
    employeesFinal = this.corporation.employees;
  }
  var raisedFinal: any;
  if(this.corporation.totalMoneyRaised == null || this.corporation.totalMoneyRaised == ""){
    raisedFinal = "Information not provided by the startup"
  }else{
    raisedFinal = this.corporation.totalMoneyRaised;
  }
  var teamFinal: any;
  if(this.corporation.background == null || this.corporation.background == ""){
    teamFinal = "Information not provided by the startup"
  }else{
    teamFinal = this.corporation.background;
  }
  var compadvFinal: any;
  if(this.corporation.advantage == null || this.corporation.advantage == ""){
    compadvFinal = "Information not provided by the startup"
  }else{
    compadvFinal = this.corporation.advantage;
  }
  var casestudyFinal: any;
  if(this.corporation.caseStudy == null || this.corporation.caseStudy == ""){
    casestudyFinal = "Information not provided by the startup"
  }else{
    casestudyFinal = this.corporation.caseStudy;
  }
  var websiteFinal: any;
  if(this.corporation.website == null || this.corporation.website == ""){
    websiteFinal = "Information not provided by the startup"
  }else{
    websiteFinal = this.corporation.website;
  }
  var tagsFinal: any;  
  if(this.corporation.tags == null || this.corporation.tags == ""){
    tagsFinal = "Information not provided by the startup"
  }else{
    tagsFinal = this.corporation.tags;
  }
...
/*   pdfMake.fonts = {
  ...
  var dd = {
    ...
    header: [{columns: [
      {
        width: 685,
        text: this.corporation.corporationName,
        style: 'header',
        alignment: 'left',
        color:'white',
        margin: [50,25,0,0]
    ...
    },
    {
      ...
      columns: [{width:'*',margin: [10,10,20,0],text: 'corporation WEBSITE', style: 'titleBigBlue'},{width:'*',margin: [10,10,20,0],text: 'TAGS', style: 'titleBigBlue'}],      
    },{
...
/*     pageBreakBefore: function(currentNode, followingNodesOnPage, nodesOnNextPage, previousNodesOnPage) {
  ...
  pdfMake.createPdf(dd).download(this.corporation.corporationName+' Executive Summary.pdf');
