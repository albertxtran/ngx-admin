...
@Component({
  selector: 'top100',
  styleUrls: ['./top100.scss'],
  templateUrl: './top100.html',
})
...
export class Top100Component implements OnInit, OnDestroy  {
  ...
  constructor(private route: ActivatedRoute, private _top100Service: Top100Service, public _toasterService: ToasterService, vcr: ViewContainerRef, private dialogService:DialogService, private router: Router) {
...
/*       pdfMake.fonts = {
  ...
  removeTop100(id:Number) {
    ...
    error => {
      this.unsetOverlay();
      this._toasterService.showError("Could not remove Top 100, please try again!", "Error", 4000)}, 
      () =>{
        let trigger = false;
        for(var i = 0; i < this.companies.length; i++){
