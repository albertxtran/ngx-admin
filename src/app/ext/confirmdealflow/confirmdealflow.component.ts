...
constructor(private route: ActivatedRoute, private _confirmDealflowService: ConfirmDealflowService, public _toasterService: ToasterService, vcr: ViewContainerRef, private router: Router) {
      ...
      this._confirmDealflowService.getDealflowStartup(this.dealflowname).subscribe(data => {this.dealflow_startup = data 
  ...
  getUserById(user_id:Number, name: number, dealflow: any){
    this.error = false;
    this._confirmDealflowService.getUserById(user_id).map(res => {
    // If request fails, throw an Error that will be caught
    if(res.status == 204) {
      this.loading = false;
      this.error = true;
...
getVenturesById(id: Number, count: number, length: number){
  ...
  this.loading = true;
  this._confirmDealflowService.getVenturesById(id).map(res => {
    // If request fails, throw an Error that will be caught
    if(res.status == 204) {
      this.loading = false;
      this.error = true;
...
removeDealflow(id:Number, dealflowname:String) {
  ...
  this._confirmDealflowService.removeFromDealflow(id,dealflowname).subscribe(data => this.dealflowstartup = data,
  error => {
...
selectPriority(id:Number, dealflow_name: String, priority: String, index: number) {
  ...
  this._confirmDealflowService.selectPriority(id,dealflow_name,priority).subscribe(data => this.dealflowstartup = data,
  error => {
...
SubmitTop20(){
  ...
  this._confirmDealflowService.updateDealflowState(this.dealflow.id,"Review").map(res => {
    // If request fails, throw an Error that will be caught
